...
def color_mode_callback(request):
    ...
    if hasattr(color_mode, 'is_inverted') and color_mode.is_inverted:
        with MappedArray(request, "main") as m:
            # Invert ONLY the first 3 channels (R, G, B) so we don't invert Alpha (making it invisible)
            m.array[..., :3] = ~m.array[..., :3]
            
    elif hasattr(color_mode, 'index') and COLOR_MODES[color_mode.index] != None:
        name, dark, light = COLOR_MODES[color_mode.index]
        with MappedArray(request, "main") as m:
            # Determine if we are working with 3 or 4 channels
            channels = m.array.shape[2] if len(m.array.shape) == 3 else 1
            
            if channels == 4:
                buffer = cv2.cvtColor(m.array, cv2.COLOR_BGRA2GRAY)
            else:
                buffer = cv2.cvtColor(m.array, cv2.COLOR_BGR2GRAY)
                
            heatmap = cv2.applyColorMap(buffer, colormap(dark, light))
            
            # If our original array has 4 channels, convert heatmap back to 4 channels
            if channels == 4:
...
# Change to given scale factor dynamically centered on the v3 sensor
def scale(new_factor):
    ...
    # Legacy fallback for Pi 3 / older OS (retained for safety)
    if hasattr(camera, 'crop'):
        diameter = min(1 / factor, 1)
        crop_x = (1.0 - diameter) / 2.0
        crop_y = (1.0 - diameter) / 2.0
        camera.crop = (crop_x, crop_y, diameter, diameter)
        return

    # -- Pi 5 / Camera Module v3 Logic Below --
    
    screen_w, screen_h = screen
    screen_ratio = screen_w / screen_h
    
    # 1. Get the absolute maximum bounds of the camera sensor
    try:
        # For Camera Module v3, this is usually (0, 0, 4608, 2592)
        sensor_x, sensor_y, sensor_w, sensor_h = camera.camera_properties['ScalerCropMaximum']
    except Exception as e:
        print(f"Warning: Failed to read ScalerCropMaximum. Using v3 defaults. Error: {e}")
        # Safe fallback for the IMX708 v3 sensor
        sensor_x, sensor_y, sensor_w, sensor_h = 0, 0, 4608, 2592

    # 2. Calculate the dimensions of the crop window
    crop_w = int(sensor_w / factor)
    crop_h = int(crop_w / screen_ratio)

    # Prevent the crop height from exceeding the physical sensor boundaries
    if crop_h > sensor_h:
        crop_h = sensor_h
        crop_w = int(crop_h * screen_ratio)

    # 3. Calculate the top-left coordinate to keep the box perfectly centered
    new_x = sensor_x + (sensor_w - crop_w) // 2
    new_y = sensor_y + (sensor_h - crop_h) // 2

    # 4. Apply the crop
    window = (new_x, new_y, crop_w, crop_h)
    print(f"Applying zoom factor {factor}x. Crop window: {window}")
    ...
    # 5. Tell the v3 camera's autofocus lens to target our newly cropped area
    ...
    try:
        ...
        if PREVIEW_MODE == 'auto':
            if os.getenv('DISPLAY'):
                preview_candidates = [
                    ('QTGL', Preview.QTGL, 'XRGB8888'),  # Changed from RGB888
                    ('DRM', Preview.DRM, 'XBGR8888')     # Changed from BGR888
                ]
            else:
                preview_candidates = [
                    ('DRM', Preview.DRM, 'XBGR8888'),
                    ('QTGL', Preview.QTGL, 'XRGB8888')
                ]
